...
def sample_pcd(point_cloud, num_points=20000):
...
def local_2_global_no_overlap(path, scale, R, T):
    ...
    '''
    # global 
    pose_g = np.load(path[0].joinpath("cams2world.npy"), allow_pickle=True).item()
    # 读取K
    ...
    pcd_g = o3d.io.read_point_cloud(str(path[0].joinpath('points3d_sparse.ply')))
    ...
    for i in tqdm(range(1, len(path)), desc="merge scene ..."):
        ...
        pcd_l = o3d.io.read_point_cloud(str(path[i].joinpath('points3d_sparse.ply')))
...
def local_2_global(path, scale, R, T):
    ...
    for i in tqdm(range(len(path)-1), desc="merge scene ..."):
        ...
        pose_g = np.load(path[i+1].joinpath("cams2world.npy"), allow_pickle=True).item()
        # 读取K
    ...
    pcd = sample_pcd(pcd_merge, num_points=100000)
